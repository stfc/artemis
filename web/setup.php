...
if ($db) {
  ...
  if ($nodes) {
    echo "<table>\n";
    echo "<tr><th>IP Address</th><th>Module</th><th>Object Class</th><th>&nbsp;</th></tr>\n";
    foreach($nodes as $n) {
      echo "<tr><td>{$n["ip"]}</td><td>{$n["module"]}</td><td>{$n["object"]}</td><td><img src=\"icons/actions/list-remove.png\" onclick=\"removeNode('{$n["ip"]}')\" /></td></tr>\n";
    }
    echo "</table>\n";
  ...
  echo "<h2>Probes</h2>\n";
  $probes = $db->query('SELECT * FROM probes');
  if ($probes) {
    echo "<table>\n";
    echo "<tr><th>ID</th><th>Name</th><th>x</th><th>y</th><th>w</th><th>h</th><th>&nbsp;</th></tr>\n";
    foreach($probes as $p) {
      echo "<tr>";
      echo "<td>{$p["id"]}</td>";
      echo "<td>{$p["name"]}</td>";
